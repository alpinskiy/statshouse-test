...
func msgpackUnmarshalStatshouseMetric(m *gen2.MetricBytes, buf []byte) ([]byte, error) {
	m.Reset()
	var err error
	var field []byte
	...
	for numFields > 0 {
		...
		case "tags":
			...
			if cap(m.Tags) >= int(numTags) {
				...
				m.Tags = make([]gen2.DictionaryFieldStringBytes, numTags)
			}
			for i := range m.Tags {
...
func msgpackUnmarshalStatshouseAddMetricBatch(mb *gen2.BatchBytes, buf []byte) ([]byte, error) {
	mb.Reset()
	var err error
	var field []byte
	var numFields uint32
	...
	for numFields > 0 {
		...
		case "metrics":
			...
			if cap(mb.Metrics) >= int(numMetrics) {
				...
				mb.Metrics = make([]gen2.MetricBytes, numMetrics)
			}
